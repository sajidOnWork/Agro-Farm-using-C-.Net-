        ...
        private void Button8_Click(object sender, EventArgs e)
        {
            if (textBox2.Text.Length > 0)
            {
                try
                {
                    m.Close();
                    m.Open();
                    SqlDataAdapter cmd = new SqlDataAdapter("select * from [dbo].[treatment]  where Symptom like '%" + textBox2.Text + "%'", m);
                    DataTable dt = new DataTable();
                    cmd.Fill(dt);
                    if (dt.Rows.Count > 0)
                    {
                        foreach (DataRow item in dt.Rows)
                        {
                            ...
                            MessageBox.Show("Symptom: " + item[2] + "\nDisease Name: " + item[1] + "\nUse this Pesticide named " + item[3] + "\nUse this Fertilizer named " + item[4]);
                            textBox2.Clear();
