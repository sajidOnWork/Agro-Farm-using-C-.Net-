    ...
    public partial class EmployeeInformation : Form
    {
        ...
        private void Button7_Click(object sender, EventArgs e)
        {
            try
            {
                m.Close();
                m.Open();
                SqlCommand cmd = new SqlCommand("delete from [dbo].[employee] where Eid='" + textBox1.Text + "'", m);
                ...
                if (a > 0)
                {
                    ...
                    this.Hide();
                    EmployeeInformation f1 = new EmployeeInformation();
                    f1.Show();
                }
                else
                {
        ...
        private void EmployeeInformation_Load(object sender, EventArgs e)
        {
            try
            {
                m.Close();
                m.Open();
                SqlDataAdapter cmd = new SqlDataAdapter("select * from [dbo].[employee]", m);
        ...
        private void Button8_Click(object sender, EventArgs e)
        {
            try
            {
                m.Close();
                m.Open();
                SqlCommand cmd = new SqlCommand("update [dbo].[employee] set FullName = '" + textBox2.Text + "', Address = '" + textBox3.Text + "', Phone = '" + textBox4.Text + "', Position = '" + textBox5.Text + "', Salary = '" + textBox6.Text + "'  where Eid='" + textBox1.Text + "'", m);
                int a = cmd.ExecuteNonQuery();
                if (a > 0)
                {
